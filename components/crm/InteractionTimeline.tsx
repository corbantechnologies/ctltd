    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        ...
        <CreateInteractionModal 
          ...
          partnerId={partnerId}
          trigger={
            <button className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-blue-600 transition-all shadow-lg active:scale-95">
          ...
          {interactions.map((interaction) => {
            ...
            return (
              <div key={interaction.id} className="relative pl-12 group">
                {/* Timeline Dot/Icon */}
                <div className={cn(
                  "absolute left-0 top-0 w-10 h-10 rounded-xl border flex items-center justify-center transition-transform group-hover:scale-110 group-hover:rotate-12 z-10",
                  interactionColors[interaction.interaction_type]
                )}>
                  <Icon className="w-5 h-5" />
                </div>

                <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm group-hover:shadow-md transition-all group-hover:border-slate-200">
