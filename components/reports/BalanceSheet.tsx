...
export function BalanceSheetReport({ data }: { data: BalanceSheet }) {
    const RowSection = ({ title, section, isTotal = false }: { title: string; section: SectionData, isTotal?: boolean }) => (
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-2 font-mono text-sm px-2 py-1 ${isTotal ? "font-bold text-black border-t border-black/10 mt-1 pt-2" : "text-corporate-muted"}`}>
    ...
    const ComplexSection = ({ title, current, non_current, total }: { title: string; current: SectionData, non_current: SectionData, total: number }) => (
        <div className="mb-4">
            <h3 className="text-xs font-semibold uppercase text-corporate-foreground mb-2 flex items-center gap-2">
                {title}
                <div className="h-px flex-1 bg-border/50"></div>
            </h3>
            <div className="hidden md:grid grid-cols-3 gap-2 md:gap-2 text-[10px] font-bold text-corporate-muted/60 mb-2 px-3">
                <div>Debit</div>
                <div>Credit</div>
    ...
    return (
        <Card className="h-full rounded">
            <CardHeader>
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <CardDescription className="font-bold text-xs uppercase mt-1">{data.division} • As of {data.as_of_date}</CardDescription>
            ...
            <CardContent>
                <ComplexSection title="Assets" current={data.assets.current} non_current={data.assets.non_current} total={data.assets.total} />
                <hr className="my-2 border-black/10" />
                <ComplexSection title="Liabilities" current={data.liabilities.current} non_current={data.liabilities.non_current} total={data.liabilities.total} />
                <hr className="my-2 border-black/10" />
                <SimpleSection title="Equity" section={data.equity} />
                ...
                <div className="pt-2">
                    <div className="flex justify-between items-center mb-2 text-sm bg-corporate-primary/5 px-2 py-1 rounded">
                        ...
                        <span className="font-mono font-bold text-corporate-primary">{formatNumber(data.total_assets)}</span>
                    </div>
